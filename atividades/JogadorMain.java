...
//	QUESTÃO 04
		...
		for (int i = 0; i < listaJogador.size(); i++) {
			...
			System.out.println(aux.getPosicao() + ": " + aux.getId() + " " + "-" + " " + aux.getNome() + " " + "("
					+ aux.getApelido() + ")" + " " + "-" + " " + aux.parseDate(aux.getDataNascimento()) + " "
					+ "CONDIÇÃO: " + aux.verJogo());
...
//		QUESTÃO 5
		...
		for (int i = 0; i < novoTime.getNoTime().size(); i++) {
			...
			System.out
					.println(novoTime.getNoTime().get(i).getQualidade() + " - " + novoTime.getNoTime().get(i).getId());
		...
		for (int i = 0; i < novoTime.getRelacionados().size(); i++) {
			...
			System.out.println(novoTime.getRelacionados().get(i).qualidade + " - "
					+ novoTime.getRelacionados().get(i).posicao + " - " + novoTime.getRelacionados().get(i).id);
		...
		System.out.println();
		System.out.println(novoJogo.mandante.nome + " " + novoJogo.placarMandante + " VS " + novoJogo.placarVisitante
				+ " " + novoJogo.visitante.nome);
		System.out.println();
		...
		for (int i = 0; i < 11; i++) {
			System.out.println(
					novoTime.getRelacionados().get(i).qualidade + " - " + novoTime.getRelacionados().get(i).id);
		}
		...
		for (int i = 0; i < 11; i++) {
			System.out.println(novoTime.getRelacionados().get(i).id + " - " + novoTime.getRelacionados().get(i).cartoes
					+ " - " + novoTime.getRelacionados().get(i).verJogo());
		}
